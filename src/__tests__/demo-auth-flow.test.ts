...
vi.mock("@/lib/demo-account", () => ({
  ...
  isDemoIdentity: (value: { id?: string | null; email?: string | null } | null | undefined) => value?.id === "demo-user" || value?.email === "demo@intelly.local",
}))
...
describe("Demo authentication provider", () => {
  it("uses a real credentials provider with a dedicated identity", async () => {
    mocks.ensureDemoAccount.mockResolvedValue({
      user: { id: "demo-user", email: "demo@intelly.local", name: "Intelly Demo User", image: null, role: "USER", companyRole: "Business Analyst", emailVerified: new Date() },
    ...
    expect(user).toEqual(expect.objectContaining({ id: "demo-user", email: "demo@intelly.local", isDemo: true, demoProjectId: "demo-project-intelly" }))
