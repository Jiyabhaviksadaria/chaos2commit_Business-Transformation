...
export function BusinessAnalysisView({ projectId, language, digitalMaturity, aiReadiness, discoveryCompleteness, onScoreUpdate, onOpenAi }: BusinessAnalysisViewProps) {
  ...
  useEffect(() => {
    if (session?.user?.isDemo || (typeof window !== "undefined" && window.location.search.includes("demo=true"))) {
      setIsDemoMode(true)
    }
  }, [session?.user?.isDemo])
  ...
  const generateAnalysis = async (instructions?: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        setIsDemoMode(true)
        toast.info("Showing Business Analysis board while discovery is in progress.")
        return
      }
      ...
      setIsDemoMode(true)
      toast.info("Showing Business Analysis board.")
    } finally {
      setGenerating(false)
    }
  }
  ...
  return <div className="space-y-6">
    ...
    </> : <Card className="bg-white border-[#E5DFD4] rounded-[26px] shadow-sm"><CardContent className="py-10 text-center space-y-4"><div className="h-12 w-12 rounded-2xl bg-[#FEE895] flex items-center justify-center mx-auto text-neutral-900"><Sparkles className="h-6 w-6" /></div><div><p className="text-base font-extrabold text-neutral-900">Business Analysis Dashboard</p><p className="text-xs text-neutral-500 mt-1 max-w-md mx-auto">Explore the interactive transformation board with business insights or generate custom AI analysis.</p></div><div className="flex flex-wrap items-center justify-center gap-3 pt-2"><Button onClick={loadDemo} className="bg-[#18181C] text-white rounded-full text-xs font-bold gap-2 px-6"><Sparkles className="w-4 h-4 text-[#F472B6]" /> Open Dashboard Now</Button><Button onClick={() => generateAnalysis()} disabled={generating} variant="outline" className="border-[#E5DFD4] rounded-full text-xs font-bold gap-2">{generating ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />} Generate AI Analysis</Button></div></CardContent></Card>}
