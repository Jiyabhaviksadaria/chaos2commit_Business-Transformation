...
export function DiscoveryLoop({ projectId, language, onScoreUpdate }: DiscoveryLoopProps) {
  ...
  return <Card className="bg-white border-[#E5DFD4] rounded-[26px] shadow-sm">
    ...
    <CardContent className="pt-6 space-y-5">
      ...
      {(confirmedFacts.length > 0 || currentProcess.length > 0 || observedProblems.length > 0 || rootCauses.length > 0 || unknowns.length > 0 || constraints.length > 0) && <div><div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-extrabold text-neutral-900">INTELLY UNDERSTANDING</h3><span className="text-[10px] text-neutral-500">Only evidence-backed findings are shown</span></div><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><UnderstandingSection title="Confirmed facts" values={confirmedFacts} tone="confirmed" /><UnderstandingSection title="Current process" values={currentProcess} /><UnderstandingSection title="Observed problems" values={observedProblems} /><UnderstandingSection title="Potential root causes" values={rootCauses} tone="confirmed" /><UnderstandingSection title="Unknown" values={unknowns} tone="unknown" /><UnderstandingSection title="Constraints" values={constraints} /></div>{evidence.length > 0 && <div className="mt-3 rounded-2xl border border-[#E5DFD4] bg-white p-3"><p className="text-[10px] font-extrabold uppercase tracking-wider text-neutral-500">Evidence collected</p><div className="mt-2 space-y-1.5">{evidence.map((item, index) => <p key={`${item.statement}-${index}`} className="text-xs text-neutral-700">✓ {item.statement} <span className="text-[10px] text-neutral-400">({item.source})</span></p>)}</div></div>}</div>}
