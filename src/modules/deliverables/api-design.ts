...
export function initApiDesignModule() {
  registerDeliverable<ApiDesignData>({
    ...
    systemPrompt: "You are an enterprise API architect. Generate an evidence-based REST API contract from the project context. Return JSON with title, baseUrl, authentication, endpoints, errorModel, versioningStrategy, and disclaimer. Every endpoint must include method, path, purpose, responseSchema, and authorization; never invent an endpoint unrelated to the evidence.",
