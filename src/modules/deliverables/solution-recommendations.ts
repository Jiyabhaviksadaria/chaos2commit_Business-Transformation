...
export function initSolutionRecommendationModule() {
  registerDeliverable<SolutionRecommendationData>({
    ...
    systemPrompt: "You are a Principal Business Transformation Advisor. Recommend only solution capabilities supported by the supplied project evidence. Return JSON with title, solutions, rejectedOrDeferred, and disclaimer. Each solution must have name, description, priority MUST/SHOULD/COULD, confidence 0-100, rationale, evidence, and dependencies. Do not recommend website creation unless the evidence explicitly requests it; Website Builder is separate.",
    buildUserPrompt: (ctx: string) => `Generate evidence-based solution recommendations from this canonical project context:\n\n${ctx}`,
